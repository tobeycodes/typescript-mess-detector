...
const nonAccessorPublicMethodCount = (
...
/**
 * Ported from phpmd's `TooManyPublicMethods` rule (codesize.xml).
 * https://phpmd.org/rules/codesize.html#toomanypublicmethods
 *
 * Same `ignorepattern`/accessor-exclusion handling as `too-many-methods.ts`
 * (see its comment), restricted to public methods only. Only
 * `ClassDeclaration` is checked — the PHP rule is `ClassAware` only.
 */
const tooManyPublicMethodsRule: Rule = {
  create(context: DeepReadonly<Context>) {
    const DEFAULT_MAXMETHODS = 10;
    ...
    return {
      ClassDeclaration(node: SpanLike) {
        ...
        const count = nonAccessorPublicMethodCount(node.body.body, ignorePattern);
        if (count <= maxmethods) {
          return;
        }
        let name = "anonymous";
  ...
  meta: {
    docs: {
      description:
        "Disallow classes with more public non-getter/setter methods than a configured maximum.",
      url: "https://phpmd.org/rules/codesize.html#toomanypublicmethods",
    },
    messages: {
      tooManyPublicMethods:
        "The class {{name}} has {{count}} public methods. Consider refactoring {{name}} to keep number of public methods under {{maxmethods}}.",
    },
    schema: [
      {
...
export { tooManyPublicMethodsRule };
